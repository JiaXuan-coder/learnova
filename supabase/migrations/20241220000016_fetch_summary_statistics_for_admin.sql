-- Get total number of students in the LMS
SELECT COUNT(DISTINCT s.student_id) AS total_student_number
FROM Student s;

-- Get total number of teachers in the LMS
SELECT COUNT(DISTINCT t.teacher_id) AS total_teacher_number
FROM Teacher t;
...
FROM Lesson l
WHERE l.status = 'published'::lesson_status;

-- Get total number of lessons draft
SELECT COUNT(DISTINCT lesson_id) AS total_lessons_draft
FROM Lesson l
WHERE l.status = 'draft'::lesson_status;

-- Get total number of lessons archive
SELECT COUNT(DISTINCT lesson_id) AS total_lessons_archive
FROM Lesson l
WHERE l.status = 'archived'::lesson_status;
...
FROM Classroom cl
WHERE (cl.start_date + (cl.duration || ' weeks')::interval) > NOW();
...
-- Full retrieval of courses summary statistics --
